          ...
          <div className="divide-y divide-slate-100">
            {filteredFeedbacks.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <h3 className="text-sm font-extrabold text-slate-900">No se encontraron opiniones</h3>
                <p className="text-xs text-slate-600 mt-1 font-medium">Prueba cambiando los filtros de búsqueda o estrellas.</p>
              </div>
            ) : (
                ...
                return (
                  ...
                  >
                      ...
                      {fb.positiveHighlights && fb.positiveHighlights.length > 0 && (
                        <div className="flex items-center gap-1.5 flex-wrap pt-1">
                          {fb.positiveHighlights.map((highlight) => (
                            <span
                              key={highlight}
                              className="text-[10px] bg-emerald-50 text-emerald-800 px-2 py-0.5 rounded border border-emerald-300 font-bold"
                            >
                              {highlight}
                            </span>
                          ))}
                    ...
                    <div className="flex items-center md:flex-col md:items-end justify-between gap-3 shrink-0">
                      ...
                      <Link
                        href={`/feedback/${fb.id}`}
                        className="px-3.5 py-2 bg-white hover:bg-slate-100 text-xs font-bold text-slate-800 border border-slate-300 rounded-xl transition-colors shadow-2xs flex items-center gap-1"
                      >
